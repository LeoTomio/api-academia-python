...
def user_autentication(cpf, password, session):
...
async def create_user(user_schema: UserSchema, session: Session = Depends(get_session)):
    ...
    new_person = Person(
        name=user_schema.person.name,
        age=user_schema.person.age,
        email=user_schema.person.email,
        phone=user_schema.person.phone,
        type=user_schema.person.type,
        is_active=user_schema.person.is_active if user_schema.person.is_active is not None else True,
    )
