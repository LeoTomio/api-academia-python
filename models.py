...
class Person(Base):
    ...
    plan_id = Column("plan_id", ForeignKey("plan.id"))
    ...
    measures = relationship("Measure", back_populates="person")  # 1:N
...
class Measure(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, nullable=False)
    measure_type = Column("measure_type", String)
    value = Column("value", Float)
    measure_date = Column("measure_date", DateTime(timezone=True))
    person_id = Column(UUID(as_uuid=True), ForeignKey("person.id", ondelete="CASCADE"), nullable=False)
